...
import { CauseListValues } from 'src/app/shared/search/models/causelist-values.model';
...
export class SearchService implements OnDestroy {
  ...
  private searchCauseListPath = 'cisresult';
  /**
   * The ResponseParsingService constructor name
   */
  private parser: GenericConstructor<ResponseParsingService> = SearchResponseParsingService;
  private parserCause: GenericConstructor<ResponseParsingService> = CauseListValueResponseParsingService;
  ...
  getEndpointCauseList(searchOptions?: PaginatedSearchOptions): Observable<string> {
    return this.halService.getEndpoint(this.searchCauseListPath).pipe(
      map((url: string) => {
        if (hasValue(searchOptions)) {
          return (searchOptions as PaginatedSearchOptions).toRestUrl(url);
        } else {
  ...
  getEndpointCauseListCaseFileMigrate(searchOptions?: PaginatedSearchOptions): Observable<string> {
    return this.halService.getEndpoint(this.searchCauseListPath).pipe(
      map((url: string) => {
        url = url + "/searchCaseFile";
        if (hasValue(searchOptions)) {
          return (searchOptions as PaginatedSearchOptions).toRestUrl(url);
        } else {
          return url;
  ...
  getEndpointCauseListCaseFileMigrateUpdate(searchOptions?: PaginatedSearchOptions): Observable<string> {
    return this.halService.getEndpoint(this.searchCauseListPath).pipe(
      map((url: string) => {
        url = url + "/migrateCaseFile";
        if (hasValue(searchOptions)) {
          return (searchOptions as PaginatedSearchOptions).toRestUrl(url);
        } else {
          return url;
  ...
  */
  searchCauseList(searchOptions?: PaginatedSearchOptions, responseMsToLive?: number, useCachedVersionIfAvailable = false, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<CauseListValues>[]): Observable<RemoteData<CauseListValues>> {
    const href$ = this.getEndpointCauseList(searchOptions);
    ...
    const sqr$ = href$.pipe(
      switchMap((href: string) => this.rdb.buildFromHref<CauseListValues>(href))
    );

    return this.directlyAttachIndexableObjectsCauseList(sqr$, useCachedVersionIfAvailable, reRequestOnStale, ...linksToFollow);


  }
  searchCauseList_UPDATE_CaseFile(searchOptions?: PaginatedSearchOptions, responseMsToLive?: number, useCachedVersionIfAvailable = false, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<CauseListValues>[]): Observable<RemoteData<CauseListValues>> {
    const href$ = this.getEndpointCauseListCaseFileMigrateUpdate(searchOptions);
    ...
    href$.pipe(
      ...
      map((href: string) => {
        ...
        if (isNotEmpty(args)) {
          ...
          console.log("URL>>>>>", new URLCombiner(href, `?${args.join('&')}`).toString())
          return new URLCombiner(href, `?${args.join('&')}`).toString();
        } else {
          return href;
        }
  ...
  searchCauseListCaseFile(searchOptions?: PaginatedSearchOptions, responseMsToLive?: number, useCachedVersionIfAvailable = false, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<CauseListValues>[]): Observable<RemoteData<CauseListValues>> {
    const href$ = this.getEndpointCauseListCaseFileMigrate(searchOptions);
  ...
  protected directlyAttachIndexableObjectsCauseList(sqr$: Observable<RemoteData<CauseListValues>>, useCachedVersionIfAvailable = true, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<CauseListValues>[]): Observable<RemoteData<CauseListValues>> {
    return sqr$.pipe(
      switchMap((resultsRd: RemoteData<CauseListValues>) => {
        return [resultsRd];
