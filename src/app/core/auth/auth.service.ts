...
export class AuthService {
  ...
  public submitLoginCreds(email: string, password: string): Observable<RemoteData<any>>{
    const body = (`password=${encodeURIComponent(password)}&user=${encodeURIComponent(email)}`);
    const options: HttpOptions = Object.create({});
    let headers = new HttpHeaders();
    headers = headers.append('Content-Type', 'application/x-www-form-urlencoded');
    options.headers = headers;

    return this.authRequestService.postToEndpoint('loginWithOTP?user=' + email.trim() + "&password=" + password.trim(), {}, options);
     
  }
  public validateOTP(email: string,password:string, opt: string): Observable<RemoteData<any>> {
    // Determine if the user has an existing auth session on the server
    const options: HttpOptions = Object.create({});
    let headers = new HttpHeaders();
    headers = headers.append('Accept', 'application/json');
    options.headers = headers;
    // options.withCredentials = true;
    return this.authRequestService.postToEndpoint('validateOTP?user=' + email.trim() + "&password=" + password.trim() +"&otp=" + opt.trim(), {},options);
  }
