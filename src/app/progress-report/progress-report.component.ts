import { Component, OnInit, Injectable, ChangeDetectorRef, Output, EventEmitter,Input } from '@angular/core';
...
export class ProgressReportComponent implements OnInit {
  ...
  getresult(): void{
    let userid = "";
    if (this.finduser != undefined || this.finduser != null) {
      userid=this.finduser.id
    } 
     this.loder = true;
  ...
  getepople(event) {
    if (event.target.value.length > 1) { 
      ...
      let query: string = String(event.target.value);
