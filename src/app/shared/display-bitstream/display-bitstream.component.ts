...
export class DisplayBitstreamComponent implements OnInit {
  ...
  firstPDF:boolean = false;
  secondPDF:boolean = false;
  constructor(
  ...
  initialize(): void {
    ...
    this.originals$.subscribe((data) => {
      if(data && data.payload && !called) {
        this.showpdf(data.payload.page[0],data.payload.page[0].name);
        called = true;
      }
    });
  ...
  sencodview(node,name:string) {
    ...
    this.fullleftC();
      ...
      if(this.bitstreamList.length === 0 ) {
        ...
        this.cdRef.detectChanges();
        this.updateDivWidths();
        this.showpdf(node,name);
        return;
      }
      this.updateDivWidths();
      
      this.bitstreamDataService.findById(node.id).pipe(getFirstSucceededRemoteData(),
        getRemoteDataPayload(),
      ).subscribe((response: any) => {
        ...
        if (!result) {
          ...
          this.authService.getShortlivedToken().pipe(take(1), map((token) =>
            hasValue(token) ? new URLCombiner(response._links.content.href, `?authentication-token=${token}`).toString() : response._links.content.href)).subscribe((logs: string) => {
              let data = { id: response.id, href: response._links.content.href };
              let obj = { name:  name, bitstream: data, };
    ...
    this.bitstreamDataService.findById(event.id).pipe(
      ...
      this.authService.getShortlivedToken().pipe(take(1), map((token) =>
        hasValue(token) ? new URLCombiner(response._links.content.href, `?authentication-token=${token}`).toString() : response._links.content.href)).subscribe((logs: string) => {
          ...
          this.cdRef.detectChanges();
          let data = { id: response.id, href: response._links.content.href };
          let obj = {  bitstream: data,name:  name};
          this.firstPDF = true;
          this.cdRef.detectChanges();
          this.bitstreamList.push(obj); // pdfSrc can be Blob or Uint8Array
          this.documenttypeTreeService.getAnnotationData(response.id).pipe(
            catchError((error) => {
              let data = {
                bitstreamRest: {
  ...
  geFileName(name:string) {
    if (name.toLowerCase().includes("notesheet")) {
      return "NoteSheet";
    } else if (name.toLowerCase().includes("correspondence")) {
